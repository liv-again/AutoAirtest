# AI驱动自然语言测试用例执行系统 - 详细设计

> 文件前缀：260614  
> 日期：2026-06-14  
> 输入材料：`AI驱动自然语言测试用例执行系统-方案设计.md`、`test-cases.xlsx`  
...
3. 启动或唤醒被测证券 App
4. 检查 Airtest 截图能力
5. 检查 Poco 控件树 dump 能力
6. 读取 test-cases.xlsx
7. 对每条用例：
   7.1 创建用例证据目录
   7.2 保存初始截图，采集初始 Poco 证据并落盘元素摘要
   7.3 Planning Agent 基于 Skills 生成 ExecutionPlan、InterpretationRationale 和动作风险等级
   7.4 Workflow 按 ExecutionPlan 推进动作
   7.5 Execution Agent 在当前 UI 证据和纠错预算内处理执行期歧义
   7.6 每个关键动作和纠错动作后保存截图、元素摘要、ExecutionTrace 和必要运行日志
   7.7 Rule Engine 基于 Poco/OCR 证据优先处理确定性验证目标
   7.8 Verification Agent 解释证据缺口、冲突证据和人工复核原因
   7.9 必要时执行受限 Evidence Recollection，并写入 ExecutionTrace
   7.10 对数据正确性、颜色规则、证据缺口等目标标记结构化人工复核原因
   7.11 保存 RunResult
8. 生成 HTML 报告
9. 可选回写 Excel 的测试结果和备注
```
...
1. 用例目录使用 `TC_用例名称__row_N`，避免重名。
2. 截图使用三位序号，保证按时间排序。
3. 元素摘要序号与截图序号对应。
4. 所有 LLM 输入输出应保存为 JSON，便于复盘。
5. Planning Agent 的 InterpretationRationale、Execution Agent 的 ExecutionTrace、Verification Agent 的结构化判断结果必须分别落盘。
6. 报告中引用相对路径，便于打包整个 `runs` 目录。
...
4. `tools/` 是稳定工具层，未来可暴露为 MCP Server。
5. `prompts/` 独立保存，便于持续调优。
6. `skills/` 保存证券 App 领域知识、操作规范和机器可读 Skill Rules。
7. `tests/fixtures/` 保存元素摘要样例、必要的调试控件树样例和 Excel 样例，避免单元测试依赖真机。
